...
lamda_tue=31      # stochastic demand distribution
lamda_wed=15
lamda_thu=32
lamda_fri=30
lamda_sat=8
...
class InventoryEnv():
    ...
    def transition(self, x_t_1, a_t_1, d_t): ## x_t_1: current state inventory position
        ## a_t_1: action which is going to be added to the inventory
        ## d_t : demand that came today
...
## the state in which we will be is not deterministic as the demand is stochastic
 ...
 ## the demand is soething that is internal to the environment, it is not exposed to the agent 
...
## to kick start the whole thing we have the initial step
## randomly initialising the state and the action
## this is done by the environment
...
## step method is the call that the agent is going to make
    ## reward( ) and transition( ) methods are in some sense internal to the environment
    ## returns the next state acter taking action a from the current state
    def step(self, x_t_1, a_t_1):   #takes the current state and the action which would be supplied by the RL agent
    ...
    def reset(self):
        return self.state
## to use this environment all the agent needs to do is to call the initial step to get started
##  And then it will repeatedly call the step fucntion
    ## the environment simulates the MDP  
